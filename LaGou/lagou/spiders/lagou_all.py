...
from ..items import LagouItemLoader,MooclagouItem
# from MoocLagou.utils.common import get_md5
...
class LagouSpider(CrawlSpider):
    ...
    def parse_item(self, response):
        item_loader=LagouItemLoader(item=MooclagouItem(),response=response)
        item_loader.add_css('title','.job-name::attr(title)')
        item_loader.add_value('url',response.url)
        item_loader.add_value('url_object_id', 'wait')
